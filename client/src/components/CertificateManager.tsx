  ...
  return <section className="space-y-5">
    ...
    <Card className="border-border/80 shadow-sm">
      <CardHeader className="flex flex-col gap-3 border-b sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Button asChild size="sm" variant="outline" className="gap-1.5" disabled={!certificates.length}><a href={backupHref("all")}><ArchiveRestore className="h-4 w-4" />전체 백업</a></Button><Dialog open={numberRuleOpen} onOpenChange={setNumberRuleOpen}><DialogTrigger asChild><Button size="sm" variant="outline" className="gap-1.5"><Settings2 className="h-4 w-4" />번호 규칙</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>성적서 발급번호 규칙</DialogTitle><DialogDescription>자동 발급번호는 접두어-연도-순번 형식으로 생성됩니다.</DialogDescription></DialogHeader><div className="grid gap-4 py-3"><Field label="접두어"><Input value={numberRuleDraft.prefix} onChange={event => setNumberRuleDraft(current => ({ ...current, prefix: event.target.value }))} placeholder="예: KOENF-QC" /></Field><Field label="순번 자릿수"><Input type="number" min="2" max="6" value={numberRuleDraft.sequenceDigits} onChange={event => setNumberRuleDraft(current => ({ ...current, sequenceDigits: event.target.value }))} /></Field><p className="rounded-lg bg-muted px-3 py-2 text-xs text-muted-foreground">예시: {numberRuleDraft.prefix || "KOENF-QC"}-{new Date().getFullYear()}-{String(1).padStart(Number(numberRuleDraft.sequenceDigits) || 3, "0")}</p></div><DialogFooter><Button disabled={updateCertificateNumberRule.isPending || !numberRuleDraft.prefix || !numberRuleDraft.sequenceDigits} onClick={() => updateCertificateNumberRule.mutate({ prefix: numberRuleDraft.prefix.trim(), sequenceDigits: Number(numberRuleDraft.sequenceDigits) })}>규칙 저장</Button></DialogFooter></DialogContent></Dialog><Dialog open={certificateOpen} onOpenChange={setCertificateOpen}>
